...
export default function PaymentForm() {
  ...
  const PaymentContinueHandler = async () => {
    // Send both screenshot and transaction ID to API
    dispatch(asynsPaymentContinue(studentid, ScreenShot, transactionId));
    setShowConfirmation(true);
  };
      ...
      <div className="w-full max-w-5xl bg-white shadow-xl rounded-2xl overflow-hidden mt-30">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* RIGHT: UPLOAD PAYMENT PROOF + TRANSACTION ID */}
            <div className="rounded-lg border border-gray-200 p-4 flex flex-col h-full space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <Upload className="text-gray-400" size={16} /> Transaction ID *
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purplee focus:border-transparent text-sm placeholder-gray-400 transition-all"
                    maxLength={30}
                    required
                  />
              ...
              <p className="text-xs text-gray-700 font-bold">
                <strong className="text-red-400 ">Note :</strong> Kindly enter the correct Transaction ID / UTR ID of your payment. ( कृपया अपनी भुगतान की सही Transaction ID / UTR ID दर्ज करें।)
              </p>
            </div>
              </div>
              ...
              <div className="mt-auto flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={PaymentContinueHandler}
                    disabled={!isUploaded || !transactionId.trim()}
                    className={`w-full sm:w-2/3 inline-flex items-center justify-center gap-2 px-6 py-3 
                      font-medium rounded-lg text-sm sm:text-base transition-all shadow-sm
                      ${
                        !isUploaded || !transactionId.trim()
                          ? "bg-purplee/50 cursor-not-allowed opacity-50 text-white"
                          : "bg-purplee hover:bg-purplee/90 text-white shadow-md hover:shadow-lg"
                      }`}
                  >
                    Continue <ArrowRight size={16} />
                  </button>
